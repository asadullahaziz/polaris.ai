...
Ported unchanged from v1 for P2 (pure, no dependencies). `_secret_values` already
scans BOTH floor and ceiling regardless of role, so it is role-symmetric today; the
P4 responder redesign generalizes it to the *union* of a principal's private limits
across all their mandates (revisions §disclosure) and renames `role → stance`.
"""
